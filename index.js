...
async function getPresentUsernames() {
  try {
    ...
    // Get the data
    const rosterRows = await rosterSheet.getCellsInRange('C2:C118');
    const rosterPresent = rosterRows.map(row => row[0]);

    const slackRows = await channelSheet.getCellsInRange('L2:L118');
    const slackNameList = slackRows.map(row => row[0]);

    const memberIDs = await channelSheet.getCellsInRange('K2:K118');
...
async function usersVotes(body, client, context, value) {
  ...
  for (const block of blocks) {
    ...
    if (
      ...
      voters.map(e1 => {
        console.log(e1);
        allAbsentMindedVoters.forEach((username, userId) => {
      ...
      votes.push({
        ...
        text: {
          type: 'mrkdwn',
          text: value.hidden===true ? splitstring[0] : block.text.text, //if it is hidden, remove useless line about vote being hidden, if not, show entire vote block
        },
      });
      votes.push({
        type: 'context',
        elements: [{
          ...
          text: !voters.length
            ...
            : value.anonymous===true ? "Number of votes: " + voters.length.toString() : voters.map(el => {
                return `<@${el}>`;
              }).join(', '),
        }],
      });
